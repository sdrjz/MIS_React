...
function MainMethod() {
  ...
  // ✅ Fetch ReportTypeSearch Types
  useEffect(() => {
    const fetchReportTypeSearch = async () => {
      try {
        const res = await axios.get('/api/GenericDropDown/ReportTypeSearch');
        const data = res.data.data || [];
        const formatted = data.map((item) => ({ value: item.Id, label: item.Label }));
        setReportTypeSearchOptions(formatted);
      } catch (err) {
      }
    };
    fetchReportTypeSearch();
  }, []);

const handleSearch = async () => {
if (!selectedReportTypeSearch) {
    ...
    <div className="p-4">
      <div className="d-flex justify-content-between align-items-center mb-3">
        <div className="flex-grow-1 text-center">
          <h3 className="report-title m-0">Bag Data</h3>
      ...
      <div className="mb-3 d-flex align-items-end flex-wrap gap-2">
         ...
         <SimpleDropdown
            dropdownOptions={ReportTypeSearchOptions} 
            placeholder="Report Type"
            onChange={(selected) => setSelectedReportTypeSearch(selected)}
          />
